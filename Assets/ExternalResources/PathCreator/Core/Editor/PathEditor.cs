	...
	public class PathEditor : Editor
	{
		...
		void DrawBezierPathInspector()
		{
			using (var check = new EditorGUI.ChangeCheckScope())
			{
				...
				if (data.showPathOptions)
				{
					...
					if (handleIndexToDisplayAsTransform != -1)
					{
						...
						using (new EditorGUI.IndentLevelScope())
						{
							...
							if (creator.transform.GetChild(handleIndexToDisplayAsTransform) != null)
								creator.transform.GetChild(handleIndexToDisplayAsTransform).position = creator.transform.position + currentPosition;
				...
				if (GUILayout.Button("Generate GameObjects from Path"))
				{
					int childCount = creator.transform.childCount;
